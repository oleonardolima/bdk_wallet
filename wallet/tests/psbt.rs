...
fn test_psbt_sign_with_finalized() {
    ...
    // FIXME: (@leonardo) It's required a deeper look on why this test fails, returning a `MissingSpendUtxo` when the finalized input is added, but succesfully signs it when it's not added.
    // add a finalized input
    // It's basically failling due to:
    // * MissingSpendUtxo: It does not pass the https://github.com/rust-bitcoin/rust-bitcoin/blob/ef5e3256dfafd84d40cabb0c09dd3f49ea117c61/bitcoin/src/psbt/mod.rs#L621-L633 spend_utxo check function, as it does not have the witness_utxo field for the added input from psbt_bip.
    eprintln!("{:#?}", psbt_bip.inputs[0]);
    eprintln!("{:#?}", psbt.inputs[0]);
...
// FIXME: (@leonardo) it's failing due to the removed `add_signer` API, it should be fixed to use an alternative testing method.
// #[test]
