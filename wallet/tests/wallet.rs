...
use bitcoin::psbt::{self, SignError, SigningErrors, SigningKeys, SigningKeysMap};
...
#[test]
#[ignore = "FIXME: further debugging and refactoring is required by this test, why is this now failing to sign for the foreign_utxo ?"]
...
#[test]
#[ignore = "FIXME: (@leonardo) why is this test is now failing ?"]
...
#[test]
#[ignore = "FIXME: further debugging and refactoring is required by this test, why it's failing to sign for the wallet_empty ?"]
fn test_taproot_sign_derive_index_from_psbt() {
    ...
    // let signer = get_wallet_signer_single(
    ...
    let signed = psbt.sign(&signer, &secp).unwrap();
    dbg!(signed);
